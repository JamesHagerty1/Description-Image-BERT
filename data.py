...
class ImageSentenceDataset(Dataset):
    def __init__(self):
        global TRAIN_FILE
        sentences, masked_ids, masked_i = [], [], []
        with open(TRAIN_FILE) as json_file:
            json_data = json.load(json_file)
        for card_entry in json_data:
            for masked_entry in card_entry["masked_sentences"]:
                sentences.append(eval(masked_entry["masked_tokens"]))
                masked_ids.append(eval(masked_entry["masked_tokens_ids"]))
                masked_i.append(eval(masked_entry["masked_tokens_i"]))
        self.sentences = torch.LongTensor(sentences)
        self.masked_ids = torch.LongTensor(masked_ids)
        self.masked_i = torch.LongTensor(masked_i)
