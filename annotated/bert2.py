# BERT that will only perform cloze test (no next sent pred)
import math
import re
import random
import numpy as np
...
maxlen = 20 # for a sentence
batch_size = 18 # sentences per batch       
max_pred = 5  # max number of tokens to mask per sentence
n_layers = 6 # number of Encoders stacked
n_heads = 12 # for multi head attention
d_model = 768 # embedding size
d_ff = 768 * 4  # 4*d_model, FeedForward dimension 
d_k = d_v = 64  # d_k is for K and Q, d_v is for V
...
def make_batch_entry(orig_sent):
    ...
    # padding
    while len(mask_pos) < max_pred:
        mask_pos.append(0)
        mask_token_ids.append(0)
